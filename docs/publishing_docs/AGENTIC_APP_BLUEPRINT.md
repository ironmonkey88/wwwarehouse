...
## 🏛 7. Data Refinery Standards (The Wong Way)
The factory does not trade in "best efforts." It trades in **Physical Proof**. All ingestion pipelines must be self-verifying.

### **The Triple-Seal Audit Mandate**
Every ingestion pipeline must implement a **Stateless Auditor** (`bin/audit_<dataset>.py`) that enforces three mandatory Gates of Parity before data is considered "Done":
1.  **Gate 1: Size (Volume)**: Total record count in the local vault must match the source exactly within the target window.
2.  **Gate 2: Shape (Distribution)**: Counts by categorical keys (e.g., `type`, `status`) must match the source exactly.
3.  **Gate 3: Sample (Moral Certainty)**: **1% of total records** (sampled randomly) must pass a **Full-Row Identity Check**, verifying every field matches the source 1:1.

### **Temporal Sovereignty (The Local Truth Standard)**
To ensure human-centric analytics and prevent "Timezone Ghosts," all refinery timestamps must adhere to:
*   **Zone Conversion**: Standardize ALL timestamps to **Eastern Time (US/Eastern)**.
*   **The NZ Seal**: Use a data type that is **NZ (No Timezone/Naive)** once converted to Eastern, preventing implicit or accidental double-conversions.
*   **Type Fidelity**: Preserve strictly—**Dates** stay **Date** types; **Timestamps** stay **Timestamp** types. No "upcasting" of simple calendar dates.

### **The TDD-First Handshake**
No feature or ingestion pipeline is considered **Done** without a corresponding **Functional Verification Gate** (e.g., `tests/quality_gates/`). Verification results must be timestamped and recorded in the [**Quality Lab**](docs/publishing_docs/quality_lab.md) as part of the Handshake Protocol.
